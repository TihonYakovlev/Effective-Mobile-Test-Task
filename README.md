...
- тестирование ключевых пользоватльских сценариев.
...
Тесты написаны на `pytest` и проверяют ключевые пользовательские сценарии.

Перед запуском тестов должен быть доступен PostgreSQL:
...
| Файл | Сценарии |
| --- | --- |
| `tests/test_auth_flow.py` | регистрация, bcrypt-хеш пароля, роль по умолчанию, login, logout, отзыв токена, soft delete |
| `tests/test_access_control.py` | `401` без токена, доступ user только к своим orders, запрет user/manager на access rules, admin update правил |

Постарался чтобы эти тесты хорошо отражали требования задания:

- пользователь не получает защищённые ресурсы без аутентификации;
- JWT после logout больше не работает;
- удалённый пользователь не может войти повторно;
- обычный user не видит чужие mock-объекты;
- manager видит все business orders, но не управляет правилами доступа;
- admin может читать и изменять access rules.

Для проверки покрытия можно запустить:
